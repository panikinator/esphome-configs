...
uint32_t IRAM_ATTR HOT AcDimmerDataStore::timer_intr(uint32_t now) {
  ...
  if (this->enable_time_us != 0 && time_since_zc >= this->enable_time_us) {
    ...
    this->disable_time_us = std::max(this->disable_time_us, time_since_zc + this->gate_enable_time);
  }
  ...
  if (this->value == 65535) {
    ...
    if (this->method == DIM_METHOD_TRAILING) {
      ...
      if (this->method == DIM_METHOD_LEADING_PULSE) {
        ...
        this->disable_time_us = std::max(this->enable_time_us + this->gate_enable_time, (uint32_t) cycle_time_us / 10);
      } else {
        this->gate_pin.digital_write(false);
...
void AcDimmer::setup() {
  ...
  this->store_.gate_enable_time = this->gate_enable_time_;
...
void AcDimmer::dump_config() {
  ESP_LOGCONFIG(TAG,
                ...
                "  Gate Enable Time: %" PRIu32 " µs",
                this->store_.min_power / 10.0f, YESNO(this->init_with_half_cycle_), this->store_.gate_enable_time);
